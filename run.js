...
if(runParams.indexOf('python') >= 0) {
  ...
  shadowsocks = spawn('ssserver', ['-m', method, '-p', '65535', '-k', tempPassword, '-t', '600', '--fast-open', '--manager-address', ssConfig ]);
} else {
  shadowsocks = spawn('ss-manager', ['-m', method, '-u', '--manager-address', ssConfig, '-t', '600', '--fast-open', '--plugin', 'obfs-server', '--plugin-opts', '"obfs=http;failover=www.bing.com:80"' ]);
}
